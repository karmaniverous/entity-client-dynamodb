...
- Enforced migration transform output entity-token matching (reject cross-entity keyed records) and updated docs to match.- Fixed TypeDoc X-Ray mode warning and added a regression test for keyed cross-entity transform rejection.- Fixed migrate error unwrapping (remove invalid `AggregateError` import) so single transform failures surface their real error message.- Fixed migrate AggregateError unwrapping by avoiding `AggregateError` symbol shadowing and unwrapping single-error aggregates via structural checks.- Fixed migrate AggregateError unwrapping to ignore `name` and unwrap any single-error `errors` wrapper (AggregateError-like) so tests and UX see the underlying error message.