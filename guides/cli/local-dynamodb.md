...
- Config-driven (preferred when configured):
  ...
  - start: run `plugins["aws/dynamodb"].local.start`, then wait for readiness before returning success.
  - status: run `plugins["aws/dynamodb"].local.status` and return its exit code (0 = healthy).
  - stop: run `plugins["aws/dynamodb"].local.stop` and return non-zero on failure.
